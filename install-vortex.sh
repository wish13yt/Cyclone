...
if [ -d "$VORTEX_DIR/APP_VAR/vortex-launcher" ]; then
    ...
    npm --prefix "$VORTEX_DIR/APP_VAR/vortex-launcher" install
...
cat > "$APPS_DIR/vortex-appsearch.desktop" << DESKTOP
[Desktop Entry]
Name=Vortex AppSearch
Comment=Search Vortex apps, mods, and tools
Exec=$APPSEARCH_DIR/appsearch.sh
Icon=$VORTEX_DIR/assets/icon.ico
Terminal=true
Type=Application
...
StartupNotify=false
DESKTOP
chmod 644 "$APPS_DIR/vortex-appsearch.desktop"

# Also place on Desktop if it exists
XDG_DESKTOP_DIR="${XDG_DESKTOP_DIR:-$HOME/Desktop}"
if [ -d "$XDG_DESKTOP_DIR" ]; then
    cp "$APPS_DIR/vortex-launcher.desktop" "$XDG_DESKTOP_DIR/" 2>/dev/null || true
    cp "$APPS_DIR/vortex-appsearch.desktop" "$XDG_DESKTOP_DIR/" 2>/dev/null || true
    chmod 755 "$XDG_DESKTOP_DIR/vortex-launcher.desktop" "$XDG_DESKTOP_DIR/vortex-appsearch.desktop" 2>/dev/null || true
fi
...
echo "  - Vortex AppSearch (app/mod search)"
echo ""
